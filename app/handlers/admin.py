...
from app.services.users import get_all_admins, set_user_admin
...
def _deny(db_user: User) -> bool:
    """Returns True if access should be denied."""
    return not db_user or not db_user.is_admin


# ── Admin Panel ──────────────────────────────────────────────────────────────

@router.callback_query(F.data == "admin_panel")
async def cb_admin_panel(call: CallbackQuery, db_user: User):
    if _deny(db_user):
...
async def cb_admin_calendars(call: CallbackQuery, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
    await call.message.edit_text(
...
async def cb_admin_cal_list(call: CallbackQuery, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
...
async def cb_admin_cal_info(call: CallbackQuery, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
...
async def cb_admin_cal_toggle(call: CallbackQuery, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
    calendar_id = int(call.data.split(":")[1])
    ...
    await call.answer("активовано ✅" if new_state else "деактивовано ⏸")
    async with async_session_maker() as session:
        cal = await get_calendar(session, calendar_id)
    type_label = "Група" if cal.type == CalendarType.GROUP else "Викладач"
...
async def cb_admin_cal_delete_confirm(call: CallbackQuery, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
    calendar_id = int(call.data.split(":")[1])
...
async def cb_admin_cal_delete_ok(call: CallbackQuery, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
    calendar_id = int(call.data.split(":")[1])
...
async def cb_admin_cal_upload(call: CallbackQuery, state: FSMContext, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
    await call.message.edit_text(
...
async def handle_calendars_file(message: Message, state: FSMContext, db_user: User):
    if _deny(db_user):
        return
...
async def cb_admin_cal_upload_confirm(call: CallbackQuery, state: FSMContext, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
...
async def cb_admin_schedules(call: CallbackQuery, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
    await call.message.edit_text(
...
async def cb_admin_sched_list(call: CallbackQuery, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
    async with async_session_maker() as session:
...
async def cb_sched_set_default(call: CallbackQuery, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
...
async def cb_admin_sched_upload(call: CallbackQuery, state: FSMContext, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
    await call.message.edit_text(
...
async def handle_schedule_file(message: Message, state: FSMContext, db_user: User):
    if _deny(db_user):
        return
    doc = message.document
    file = await message.bot.get_file(doc.file_id)
...
async def cb_sched_upload_confirm(call: CallbackQuery, state: FSMContext, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
...
async def cb_sched_override(call: CallbackQuery, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
    async with async_session_maker() as session:
...
async def cb_sched_override_pick(call: CallbackQuery, state: FSMContext, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
    sched_id = int(call.data.split(":")[1])
...
async def cb_override_date_selected(call: CallbackQuery, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
...
async def cb_admin_admins(call: CallbackQuery, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
    await call.message.edit_text(
...
async def cb_admin_add_admin(call: CallbackQuery, state: FSMContext, db_user: User):
    if _deny(db_user):
        await call.answer("⛔ Доступ заборонено", show_alert=True)
        return
    await call.message.edit_text(
...
async def handle_new_admin_id(message: Message, state: FSMContext, db_user: User):
    if _deny(db_user):
        return
    try:
